...
const ROXY_API_URL = "https://api.roxyapi.com/v1/kp_chart";
const ROXY_API_KEY = "YOUR_API_KEY"; // Admin should update this
...
interface NewKPPlanet {
  name: string;
  star_lord: string;
  sub_lord: string;
}
interface NewKPResult {
  cusps: NewKPCusp[];
  planets: NewKPPlanet[];
}
...
export default function HoroscopePage() {
  ...
  const [kpMode, setKpMode] = useState<"old" | "new" | "viku">("old");
  const [newKpResult, setNewKpResult] = useState<NewKPResult | null>(null);
  const [newKpAyanamsa, setNewKpAyanamsa] = useState<number | null>(null);
  const [oldKpResult, setOldKpResult] = useState<NewKPResult | null>(null);
  const [isNewKpLoading, setIsNewKpLoading] = useState(false);
  ...
  const handleRoxyKPCalculate = async (mode: "old" | "new") => {
    ...
    setIsNewKpLoading(true);
    if (mode === "old") setOldKpResult(null);
    else {
      setNewKpResult(null);
      setNewKpAyanamsa(null);
    }
    try {
      const payload = {
        day: d,
        ...
        lon,
        tzone: tz,
        ayanamsa: mode === "old" ? 5 : 6,
      };
      const res = await fetch(ROXY_API_URL, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${ROXY_API_KEY}`,
          "Content-Type": "application/json",
        },
        body: JSON.stringify(payload),
      });
      if (!res.ok) throw new Error(`API error: ${res.status}`);
      const data = (await res.json()) as NewKPResult;
      if (mode === "old") {
        setOldKpResult(data);
        setTimeout(
          () =>
            document
              .getElementById("kp-result")
              ?.scrollIntoView({ behavior: "smooth" }),
          100,
        );
      } else {
        setNewKpResult(data);
        // Compute New KP ayanamsa for display (different from Old KP formula)
        const utHour = hr + min / 60 - Number.parseFloat(form.tz);
        const refJd = julianDay(y, m, d, utHour);
        setNewKpAyanamsa(computeNewKPAyanamsa(refJd));
        setTimeout(
          () =>
            document
              .getElementById("new-kp-result")
              ?.scrollIntoView({ behavior: "smooth" }),
          100,
        );
      }
      ...
      toast.error(
        `KP API error: ${e instanceof Error ? e.message : String(e)}`,
      );
    } finally {
      setIsNewKpLoading(false);
    }
  };

  const handleCalculate = () => {
    // For old/new KP, also call RoxyAPI for sub-lord data (needs valid API key)
    if (kpMode === "old" || kpMode === "new") {
      handleRoxyKPCalculate(kpMode);
    }
    if (!hasServiceAccess("astro-chart")) {
      setShowGate(true);
      return;
    ...
    try {
      ...
      // Use correct ayanamsa type based on selected KP mode
      const chartAyanamsaType = kpMode === "new" ? "kp-new" : "kp-old";
  ...
  const handleTransitCalculate = () => {
    ...
    try {
      ...
      const lon = Number.parseFloat(form.lon);
      const tz = Number.parseFloat(form.tz);
      const natalTropCusps = result.cusps.map((c) => c.sidLon);
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-4 py-6">
        <Tabs defaultValue="horoscope" className="space-y-4">
          ...
          <TabsContent value="horoscope" className="space-y-6">
            ...
            >
              <div className="rounded-xl border shadow-gold bg-card p-5 space-y-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex rounded-lg border border-border overflow-hidden">
                    <button
                      type="button"
                      data-ocid="kp.toggle"
                      onClick={() => setKpMode("old")}
                      className={`px-4 py-1.5 text-xs font-semibold transition-colors ${kpMode === "old" ? "bg-[#2E8B57] text-white" : "bg-background text-muted-foreground hover:bg-muted"}`}
                    >
                      Old KP
                    </button>
                    <button
                      type="button"
                      onClick={() => setKpMode("new")}
                      className={`px-4 py-1.5 text-xs font-semibold transition-colors ${kpMode === "new" ? "bg-[#2E8B57] text-white" : "bg-background text-muted-foreground hover:bg-muted"}`}
                    >
                      New KP
                    </button>
                    <button
                      type="button"
                      onClick={() => setKpMode("viku")}
                      className={`px-4 py-1.5 text-xs font-semibold transition-colors ${kpMode === "viku" ? "bg-[#7c3aed] text-white" : "bg-background text-muted-foreground hover:bg-muted"}`}
                    >
                      Viku
                    </button>
                  </div>
                ...
                <Button
                  ...
                  disabled={isCalculating || isNewKpLoading}
                ...
                >
                  {isCalculating ? (
                    ...
                    <>
                      ...
                      {kpMode === "old"
                        ? "Calculate KP Chart (Old KP)"
                        : kpMode === "new"
                          ? "Calculate KP Chart (New KP)"
                          : "Calculate Viku Chart"}
                    </>
                  )}
                </Button>
              </div>
                  ...
                  <div className="rounded-xl border border-border bg-card px-4 py-3 space-y-2">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      >
                        ...
                        {kpMode === "new" && (
                          <>Ayanamsa: {result.ayanamsa.toFixed(4)}° (KP New)</>
                        )}
                        {kpMode === "viku" && (
                          <>Ayanamsa: {result.ayanamsa.toFixed(4)}° (Viku)</>
                        )}
                      </Badge>
                      <Button
            ...
            {((kpMode === "new" && newKpResult) ||
              (kpMode === "old" && oldKpResult)) && (
              <motion.section
                id="new-kp-result"
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
              ...
              >
                ...
                <div className="rounded-xl border border-border bg-card px-4 py-3">
                  <div className="flex flex-wrap items-center gap-x-6 gap-y-1.5">
                    <div className="flex items-center gap-1.5">
                      <Star className="w-3.5 h-3.5 text-[#2E8B57]" />
                      <span className="text-xs text-muted-foreground">
                        DOB:
                      </span>
                    ...
                    >
                      {kpMode === "old"
                        ? "Old KP System (RoxyAPI)"
                        : "New KP System (RoxyAPI)"}
                    </Badge>
                    {kpMode === "new" && newKpAyanamsa !== null && (
                      <Badge
                        variant="secondary"
                        className="bg-purple-100 text-purple-800 border-purple-300 text-xs"
                      >
                        Ayanamsa: {newKpAyanamsa.toFixed(4)}° (KP New)
                      </Badge>
                    )}
                  </div>
                </div>
                ...
                <div className="rounded-xl border border-border bg-card overflow-hidden">
                  <div className="px-4 py-3 border-b border-border bg-muted/30 flex items-center gap-2">
                    ...
                    <span className="font-semibold text-sm">
                      {kpMode === "old"
                        ? "KP Planet Sub-Lords (Old System)"
                        : "KP Planet Sub-Lords (New System)"}
                    </span>
                  </div>
                    ...
                    <Table>
                      <TableHeader>
                        <TableRow className="bg-muted/30">
                          ...
                          <TableHead className="text-xs">Star Lord</TableHead>
                      ...
                      <TableBody>
                        {(kpMode === "old"
                          ...
                          >
                            ...
                            <TableCell>{p.star_lord}</TableCell>
                            <TableCell>{p.sub_lord}</TableCell>
                          </TableRow>
                        ))}
                      </TableBody>
                    </Table>
                ...
                <div className="rounded-xl border border-border bg-card overflow-hidden">
                  <div className="px-4 py-3 border-b border-border bg-muted/30 flex items-center gap-2">
                    ...
                    <span className="font-semibold text-sm">
                      {kpMode === "old"
                        ? "KP Cusp Sub-Lords (Old System)"
                        : "KP Cusp Sub-Lords (New System)"}
                    </span>
                  </div>
                  <div className="overflow-x-auto">
                    <Table>
                      <TableHeader>
                        <TableRow className="bg-muted/30">
                          ...
                          <TableHead className="text-xs">Sign Lord</TableHead>
                          <TableHead className="text-xs">Star Lord</TableHead>
                          <TableHead className="text-xs">Sub Lord</TableHead>
                        </TableRow>
                      </TableHeader>
                      <TableBody>
                        {(kpMode === "old"
                          ...
                          >
                            ...
                            <TableCell>{c.sign_lord}</TableCell>
                            <TableCell>{c.star_lord}</TableCell>
                            <TableCell>{c.sub_lord}</TableCell>
                          </TableRow>
                        ))}
                      </TableBody>
                    </Table>
            ...
            {isNewKpLoading && (
              <div
                ...
                data-ocid="new-kp.loading_state"
              >
                <Loader2 className="w-8 h-8 mx-auto animate-spin text-[#2E8B57]" />
                <p className="text-sm text-muted-foreground mt-2">
                  {kpMode === "old"
                    ? "Fetching Old KP data..."
                    : "Fetching New KP data..."}
                </p>
              </div>
            )}

            {!result && !oldKpResult && !newKpResult && (
              <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
